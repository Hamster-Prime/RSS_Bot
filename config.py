...
def load_config() -> Optional[Dict[str, Any]]:
    if not os.path.exists(CONFIG_FILE):
        logger.error(
            f"未找到 {CONFIG_FILE}。"
            f"请将 config.json.example 复制为 {CONFIG_FILE} 并填写。"
        )
        return None
    ...
    try:
        ...
        if '/' in data_file_name or '\\' in data_file_name:
            logger.warning(
                f"config.json 中的 data_file ('{data_file_name}') "
                f"应为文件名，而不是路径。仅使用文件名部分。"
            )
        ...
        logger.error(f"解码 {CONFIG_FILE} 出错: {e}。请确保它是有效的 JSON。")
        return None
